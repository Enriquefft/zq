...
test "tryAlignChunk: spans_lookback fires when value crosses lookback/data boundary" {
    // Pretty-printed object opens in lookback, closes in data, followed by
    // a normal record. The only `\n` in lookback is at offset 7 (after the
    // prior record's closing `}`). lb_start = 8 = "{\n  \"a\":". Feeding
    // that to a fresh parser yields .need_more (object incomplete).
    // Feeding `data` then completes the object at consumed_data=4 →
    // probe returns .confirmed_in_data → tryAlignChunk returns
    // .spans_lookback(8).
    const lookback = "{\"x\":0}\n{\n  \"a\":";
    const data = " 1\n}\n{\"b\":2}";
    const r = tryAlignChunk(data, 0, lookback);
    try std.testing.expectEqual(AlignResult{ .spans_lookback = 8 }, r);
}

test "tryAlignChunk: spans_lookback rejected when no prior \\n confirms boundary" {
    // Lookback contains only mid-value bytes with no `\n` — there's no
    // candidate boundary to walk back to.
    const lookback = "  \"a\":";
    const data = " 1\n}\n{\"b\":2}";
    const r = tryAlignChunk(data, 0, lookback);
    try std.testing.expectEqual(AlignResult.in_progress, r);
...
test "tryAlignChunk: spans_lookback walks back through complete records to spanning one" {
    // Two complete records then an incomplete record start. Algorithm
    // walks back from the last `\n` (offset 23, between `2}` and `{`).
    // First probe: lb_start=24 = "{\n  \"c\":" → need_more in lookback,
    // .done in data. Returns .spans_lookback(24).
    const lookback = "{\"a\":1}\n{\"b\":2}\n{\n  \"c\":";
    const data = " 3\n}\n{\"d\":4}";
    const r = tryAlignChunk(data, 0, lookback);
    try std.testing.expectEqual(AlignResult{ .spans_lookback = 16 }, r);
}

test "feedSpanning: emits spanning record with consumed_data" {
    var p = try Parser.init(std.testing.allocator);
    defer p.deinit();

    const lb_tail = "{\n  \"a\":";
    const data = " 1\n}\n{\"b\":2}";
    const r = try parser.feedSpanning(&p, lb_tail, data, true);
    switch (r) {
        .done => |d| {
            // The spanning record is `{\n  "a": 1\n}` — that's 4 bytes of
            // data (` 1\n}`) following the lookback tail.
            try std.testing.expectEqual(@as(usize, 4), d.consumed_data);
            // Tape carries a complete object with key "a" and int 1.
            try std.testing.expect(d.tape.entries.len >= 3);
        },
        else => return error.UnexpectedSpanResult,
    }
}
