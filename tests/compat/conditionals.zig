...
test "jq:L1314 [.[] | if .foo then _yep_ else _nope_ end]" {
    ...
    try h.expectJsonEqual("[\"yep\",\"yep\",\"yep\",\"yep\",\"nope\",\"nope\",\"yep\",\"nope\"]", results[0]);
}
...
test "jq:L1322 [if 1,null,2 then 3 else 4 end]" {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("[3,4,3]", results[0]);
}
...
test "jq:L1330 [if 1 then 3,4 else 5 end]" {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("[3,4]", results[0]);
}
...
test "jq:L1334 [if null then 3 else 5,6 end]" {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("[5,6]", results[0]);
}
...
test "jq:L1370 [.[] | [.foo[] // .bar]]" {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("[[1,2],[1],[3],[42],[41]]", results[0]);
}
...
test "jq:L1374 .[] //= .[0]" {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("[\"hello\",true,\"hello\",[false],\"hello\"]", results[0]);
}
...
test "jq:L1378 .[] | [.[0] and .[1], .[0] or .[1]]" {
    ...
    try h.expectJsonEqual("[false,true]", results[1]);
    try h.expectJsonEqual("[false,true]", results[2]);
    try h.expectJsonEqual("[false,false]", results[3]);
}
...
test "jq:L1385 [.[] | not]" {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("[false,false,true,true,false,false]", results[0]);
}
