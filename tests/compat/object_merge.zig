...
test "jq:L2295 { a, $__loc__, c }" {
    ...
    try h.expectJsonEqual("{\"a\":[1,2,3],\"__loc__\":{\"file\":\"<top-level>\",\"line\":1},\"c\":{\"hi\":\"hey\"}}", results[0]);
}
...
test "jq:L2299 1 as $x | _2_ as $y | _3_ as $z | { $x, as, $y: 4, ($z): ..." {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("{\"x\":1,\"as\":8,\"2\":4,\"3\":5,\"if\":6,\"foo\":7}", results[0]);
}
...
test "jq:L2310 tojson | fromjson" {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("{\"a\":null}", results[0]);
}
...
test "jq:L2315 .[] | try (fromjson | isnan) catch ." {
    ...
    try h.expectJsonEqual("\"Invalid numeric literal at EOF at line 1,column 4 (while parsing 'NaN1')\"", results[2]);
    try h.expectJsonEqual("\"Invalid numeric literal at EOF at line 1,column 5 (while parsing 'NaN10')\"", results[3]);
    try h.expectJsonEqual("\"Invalid numeric literal at EOF at line 1,column 6 (while parsing 'NaN100')\"", results[4]);
    try h.expectJsonEqual("\"Invalid numeric literal at EOF at line 1,column 7 (while parsing 'NaN1000')\"", results[5]);
    try h.expectJsonEqual("\"Invalid numeric literal at EOF at line 1,column 8 (while parsing 'NaN10000')\"", results[6]);
    try h.expectJsonEqual("\"Invalid numeric literal at EOF at line 1,column 9 (while parsing 'NaN100000')\"", results[7]);
}
...
test "jq:L2328 try input catch ." {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("\"break\"", results[0]);
}
...
test "jq:L2337 _foo_ | try ((try . catch _caught too much_) | error) cat..." {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("\"caught just right\"", results[0]);
}
...
test "jq:L2341 .[]|(try (if .==_hi_ then . else error end) catch empty) ..." {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("\"hi there!\"", results[0]);
}
...
test "jq:L2345 try ([_hi_,_ho_]|.[]|(try . catch (if .==_ho_ then _BROKE..." {
    ...
    try h.expectJsonEqual("\"hi there!\"", results[0]);
    try h.expectJsonEqual("\"caught outside ho\"", results[1]);
}
...
test "jq:L2354 try (try error catch _inner catch _(.)_) catch _outer cat..." {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("\"inner catch foo\"", results[0]);
}
...
test "jq:L2358 try ((try error catch _inner catch _(.)_)|error) catch _o..." {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("\"outer catch inner catch foo\"", results[0]);
}
...
test "jq:L2368 {foo: _bar_} | .foo |= .?" {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("{\"foo\":\"bar\"}", results[0]);
}
...
test "jq:L2381 .[] |= try tonumber" {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("[1,3,6.7,0.89,-876,5.43,21]", results[0]);
}
...
test "jq:L2394 implode|explode" {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("[65533,0,1,2,3,1114111,65533,55295,65533,65533,57344,1,1]", results[0]);
}
...
test "jq:L2398 map(try implode catch .)" {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("[\"implode input must be an array\",\"string (\\\"a\\\") can't be imploded,unicode codepoint needs to be numeric\",\"number (null) can't be imploded,unicode codepoint needs to be numeric\"]", results[0]);
}
...
test "jq:L2402 try 0[implode] catch ." {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("\"Cannot index number with string (\\\"\\\")\"", results[0]);
}
...
test "jq:L2407 walk(.)" {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("{\"x\":0}", results[0]);
}
