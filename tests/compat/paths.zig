...
test "jq:L1110 path(.foo[0,1])" {
    ...
    try h.expectJsonEqual("[\"foo\",0]", results[0]);
    try h.expectJsonEqual("[\"foo\",1]", results[1]);
}
...
test "jq:L1123 try path(.a | map(select(.b == 0))) catch ." {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("\"Invalid path expression with result [{\\\"b\\\":0}]\"", results[0]);
}
...
test "jq:L1127 try path(.a | map(select(.b == 0)) | .[0]) catch ." {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("\"Invalid path expression near attempt to access element 0 of [{\\\"b\\\":0}]\"", results[0]);
}
...
test "jq:L1131 try path(.a | map(select(.b == 0)) | .c) catch ." {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("\"Invalid path expression near attempt to access element \\\"c\\\" of [{\\\"b\\\":0}]\"", results[0]);
}
...
test "jq:L1135 try path(.a | map(select(.b == 0)) | .[]) catch ." {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("\"Invalid path expression near attempt to iterate through [{\\\"b\\\":0}]\"", results[0]);
}
...
test "jq:L1139 path(.a[path(.b)[0]])" {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("[\"a\",\"b\"]", results[0]);
}
...
test "jq:L1143 [paths]" {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("[[0],[1],[1,0],[1,1],[1,1,\"a\"]]", results[0]);
}
...
test "jq:L1147 [_foo_,1] as $p | getpath($p), setpath($p; 20), delpaths(..." {
    ...
    try h.expectJsonEqual("\"b\"", results[0]);
    try h.expectJsonEqual("{\"bar\":42,\"foo\":[\"a\",20,\"c\",\"d\"]}", results[1]);
    try h.expectJsonEqual("{\"bar\":42,\"foo\":[\"a\",\"c\",\"d\"]}", results[2]);
}
...
test "jq:L1153 map(getpath([2])), map(setpath([2]; 42)), map(delpaths([[..." {
    ...
    try std.testing.expectEqual(@as(usize, 3), results.len);
    try h.expectJsonEqual("[null,null,2]", results[0]);
    try h.expectJsonEqual("[[0,null,42],[0,1,42],[0,1,42]]", results[1]);
    try h.expectJsonEqual("[[0],[0,1],[0,1]]", results[2]);
}
...
test "jq:L1159 map(delpaths([[0,_foo_]]))" {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("[[{\"x\":1}],[{\"bar\":2}]]", results[0]);
}
...
test "jq:L1163 [_foo_,1] as $p | getpath($p), setpath($p; 20), delpaths(..." {
    ...
    try h.expectJsonEqual("{\"bar\":false,\"foo\":[null,20]}", results[1]);
    try h.expectJsonEqual("{\"bar\":false}", results[2]);
}
...
test "jq:L1173 try delpaths(0) catch ." {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("\"Paths must be specified as an array\"", results[0]);
}
...
test "jq:L1177 del(.), del(empty), del((.foo,.bar,.baz) | .[2,3,0]), del..." {
    ...
    try h.expectJsonEqual("null", results[0]);
    try h.expectJsonEqual("{\"foo\":[0,1,2,3,4],\"bar\":[0,1]}", results[1]);
    try h.expectJsonEqual("{\"foo\":[1,4],\"bar\":[1]}", results[2]);
    try h.expectJsonEqual("{\"bar\":[1]}", results[3]);
}
...
test "jq:L1184 del(.[1], .[-6], .[2], .[-3:9])" {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("[0,3,5,6,9]", results[0]);
}
...
test "jq:L1201 pick(.a.b.c)" {
    ...
    try std.testing.expectEqual(@as(usize, 1), results.len);
    try h.expectJsonEqual("{\"a\":{\"b\":{\"c\":null}}}", results[0]);
}
