//! Leak regression for the new compiler. Cycles `compileNew` with both
...
fn compileOnceAndCheck(filter: []const u8, opts: query.Opts) !void {
    ...
    const result = query.CompiledQuery.compileNew(filter, opts, alloc) catch |e| switch (e) {
        error.OutOfMemory => {
            _ = gpa.deinit();
        ...
        error.NewCompilerNotImplemented => {
            ...
            if (check == .leak) return error.LeakedOnNotImplemented;
            return;
        },
    };
    var r = result;
...
test "new compiler: success path is leak-clean (no external vars)" {
    try compileOnceAndCheck(".", .{});
}

test "new compiler: success path is leak-clean (with external vars)" {
    const ext = [_]query.ExternalVarDecl{ .{ .name = "x" }, .{ .name = "y" } };
    try compileOnceAndCheck(".", .{ .external_vars = &ext });
}

test "new compiler: NotImplemented path is leak-clean" {
