...
| `types` | Tape, Value, Instruction, Format. 10 tests. |
...
| `query` | Lexer (7 tokens), compiler (dot-path + pipe + iterate), VM (7 opcodes), fuse pass. 26 tests. |
| `output` | Buffered Writer, pretty/compact/raw/jsonl, JSON escaping. 56 tests. |
| `pool` | Worker pool, file chunking, stream pipeline, sequencer for in-order delivery. 21 tests. |
| `c_abi` | zq_compile/zq_execute/zq_get_result/zq_free. 37 tests. |
| `main.zig` | CLI with arg parsing. 6 flags work, 11 stubbed. |
| **Total** | **233 module tests + ~50 compat tests (partial migration)** |
...
That is approximately **5% of jq's language surface**.
...
|---------|-------|-----------------|
| **Arithmetic** (`+`, `-`, `*`, `/`, `%`) | Compiler + VM | Object merge (`+`), string concat, array concat, numeric math. Unblocks ~40% of failing tests. |
| **Unary negation** (`-expr`) | Compiler + VM | `-1`, `-.foo` |
| **Comparisons** (`==`, `!=`, `<`, `>`, `<=`, `>=`) | Compiler + VM | Required for `select`, `if/then`, sorting |
| **Boolean operators** (`and`, `or`, `not`) | Compiler + VM | Required for any conditional logic |
| **Conditionals** (`if/then/elif/else/end`) | Compiler + VM | Control flow. ~20 blocked tests. |
| **Variables** (`expr as $x \| ...`) | Compiler + VM (scope stack) | Required for object construction, reduce, def, and most non-trivial filters |
| **Comma operator** (`a, b`) | Compiler + VM (generator stack) | Multiple outputs from a single expression. Required for array/object construction. |
| **Object construction** (`{a: .b, c: .d}`) | Compiler + VM | The #1 real-world use case. Shorthand `{a}` = `{a: .a}`. Computed keys `{(.k): .v}`. |
| **Array construction** (`[expr]`) | Compiler + VM | Collect generator outputs into array. `[.[] \| .name]`. |
| **String interpolation** (`"hello \(.name)"`) | Lexer + Compiler + VM | Very common in practice. Required for readable output. |
| **Recursive descent** (`..`) | Compiler + VM | Deep search. `.. \| .name?` |
...
| **`def`** (user-defined functions, including recursion) | Composability |
...
- [ ] Demonstrated 10x+ throughput on JSONL workloads vs jq
...
| **Parallel file mode** | `-P N` flag. Pool module already implements this. Wire to CLI. |
...
| Flag | Detail |
|------|--------|
| `-P N` / `--parallel N` | **zq extension.** Process JSONL with N worker threads. Default: 1. `-P 0` = auto-detect CPU count. |
...
The killer feature. Already architected (pool module), needs CLI surface.

| Item | Detail |
|------|--------|
| `-P 0` / `--parallel auto` | Auto-detect core count, split JSONL across workers |
| **In-order output guarantee** | Sequencer already ensures this. Results appear in input order regardless of which worker finishes first. |
| **Per-line error handling** | Bad line N is reported but does not kill the pipeline. Other lines continue processing. |
| **Work stealing** | File mode: byte-range chunks aligned to newline boundaries. Workers steal from shared queue. |
| **Stream pipeline** | Stdin/socket mode: dedicated IO thread reads lines, workers process in parallel. |
| **Scaling** | Near-linear scaling up to core count on JSONL. Target: 8 cores = 7x throughput. |
...
| **O(n) incremental parsing** | Parser maintains state across `feed()` calls. No re-parsing from position 0 on each chunk. Already implemented. |
| **Partial string completion** | `"hel` → `"hel"` (close the string). Already implemented via auto-close. |
...
| Item | Detail |
|------|--------|
| **i64 integers** | Already using i64 in Tape. Integers up to 2^63 - 1 are exact. |
| **Literal passthrough** | Numbers that are never modified by arithmetic are output verbatim from the source bytes. No float round-trip corruption. Requires storing the original byte range in Tape entries. |
| **i128 / bigint for arithmetic** | When i64 overflows during `+`/`-`/`*`, promote to i128 or arbitrary precision rather than silently wrapping or converting to float. |
...
| Format | Priority | Detail |
|--------|----------|--------|
| **JSONC** | High | JSON with `//` and `/* */` comments. Strip comments during parsing. Very common in config files. |
| **NDJSON** | Already supported | Newline-delimited JSON = JSONL. Already the default multi-document mode. |
| **YAML** | Medium | Read YAML input, apply jq filters, output JSON. Single-document and multi-document. |
| **TOML** | Medium | Read TOML input, apply jq filters, output JSON. |
| **CSV/TSV** | Medium | Read CSV/TSV with header row → array of objects. `--csv-input`, `--tsv-input`. |
| **MessagePack** | Low | Binary JSON. Read MessagePack, apply jq filters, output JSON or MessagePack. |
| **CBOR** | Low | Binary JSON (IETF standard). Similar to MessagePack support. |
...
| Item | Detail |
|------|--------|
| **Query plan caching** | When processing JSONL, compile the query once (already done), but also cache VM scratch buffers across records to avoid repeated allocation. |
| **Tape arena allocator** | Per-record arena that resets between records. Zero `free()` calls during steady-state JSONL processing. |
| **Output batching** | Accumulate multiple results in the 64 KB output buffer before issuing `write()`. Already implemented. |
| **mmap for large files** | Already implemented in io module. Zero-copy reads for regular files. |
...
|--------|------|------|------|
| jq compat test pass rate | 60% | 95% | 100% |
| Throughput vs jq (single-threaded, citylots.json) | 2x | 5x | 10x |
| Throughput vs jq (parallel, 1M-line JSONL, 8 cores) | — | 15x | 20x |
