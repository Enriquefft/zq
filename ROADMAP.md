...
| [x] | **Predicate pushdown into parser** | For `select(.age > 30)`, evaluate the predicate during parsing. Once `.age` is found and its value parsed, reject immediately without constructing the rest of the tape. | dsJSON (SIGMOD 2023) introduced projection trees for distributed JSON. Fusing predicate evaluation with tape construction in a bytecode VM is novel. | Full tape constructed before any filtering. VM evaluates predicate on complete value. | **Rejected (2026-05-09)** — implemented at `440ebf2`/`04c6fbf`, reverted at `af78b17`. Pattern presupposes V ≫ E (slow VM, fast parser); zq's SIMD-accelerated parse and bytecode VM make V ≈ E, so pushdown wins on wide records (-2.79%) but loses on narrow records (+2.18%) — workload-regime trade, disqualified. See `research/09-pushdown-doesnt-transfer.md` on the `research` branch. |
...
**Implementation order:** Projection pushdown first (clearest gain, simplest to measure). Predicate pushdown is closed (see Rejected verdict above) — not part of the sequence. Sparser and speculation only after profiling on real workloads justifies them. On-demand is paper-track only.
...
| [x] | **Close the streaming/file gap** | Was 3.3× on baseline 168 MB / 2M-line JSONL workload; on current main with Phase 1+2 landed it is **1.12×** (file 236 ms, redirect 236 ms, pipe 265 ms; ReleaseFast, 16-core). Fixes implemented: (1) `submit_source` dispatch routes mmap-backed stdin (`< file`) to the file/mmap path — closes the redirect gap entirely; (2) `InputSlotPool` replaces per-batch `allocator.dupe`/intermediate `batch_buf` with preallocated slots (slot_size = 2 × batch_size for tail headroom, single-record hard cap = slot_size with graceful `IoError` on oversize records, sized via `MemoryBudget.computeParams.stream_input_slots`). The residual 1.12× pipe gap is single-threaded IO-thread CPU (SIMD boundary scan + tail-carry) — irreducible without moving boundary scanning into workers, which is a separate redesign. The original `signal() → broadcast()` direction was rejected after review: `signal()` correctly wakes one worker per push and `broadcast()` would wake N-1 workers to find an empty queue. | Systematic analysis of streaming vs mmap performance in a real system, with principled fixes. | n/a — landed. | **Done — 1.6× → 1.12×** |
