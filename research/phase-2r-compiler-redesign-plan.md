...
| P24 | cat-15 | `label_expr`, `break_expr`, `until`, `while` (`recurse` builtin-call deferred — legacy parser rejects with syntax error, so adding it would be net-new functionality not redesign-parity work) | `lowerNode` else (1251) for `.label_expr`/`.break_expr`; `lowerBuiltinCall` for `until`/`while` | Backward-jump control flow + label/break unwinding share VM patch-table semantics | 1.3× |
