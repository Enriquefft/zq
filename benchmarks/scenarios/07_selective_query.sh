#!/bin/bash
# Scenario 7: Selective Query — Narrow Records (workload coverage)
# `select(.id > N) | .id` over huge.jsonl (~80B/line). Single-binary
# zq vs jq. Tracks selective-filter throughput on small records, where
# the per-record parse cost dominates and the VM trip is short.
...
# not a single array, so the filter applies to each record directly —
# no `.[]` prefix.
...
echo "# Scenario 7: Selective Query (Narrow Records)" > "$RESULT_FILE"
echo "" >> "$RESULT_FILE"
echo "**Date:** $(date -u +"%Y-%m-%d %H:%M:%S UTC")" >> "$RESULT_FILE"
echo "" >> "$RESULT_FILE"
echo "**Description:** Selective-filter throughput on narrow records (~80B/line). Workload-coverage benchmark." >> "$RESULT_FILE"
echo "" >> "$RESULT_FILE"
...
echo "" >&2
start_phase_timer "Hyperfine benchmark (selective query — narrow records)"
