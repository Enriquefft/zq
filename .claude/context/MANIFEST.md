...
| `query` | Compile jq-compatible filter expressions into bytecode `CompiledQuery`; execute against a `Tape` via lazy `ResultIterator`. Fuse pass collapses `.a \| .b` chains into single `load_path` instructions. `CompiledQuery` is immutable and safe for concurrent `execute()` calls. | Active | `error`, `types` | 2026-03-09 |
