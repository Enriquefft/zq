...
pub const builtins = [_]BuiltinInfo{
    ...
    .{ .name = "test", .arity = 1, .signature = "test(regex)", .doc = "True if the input string matches the regex. Compat: no backrefs (`(\\w+) \\1`), no lookaround (`(?=…)`, `(?<=…)`); compile errors point at the offending construct.", .category = .string },
    .{ .name = "match", .arity = 1, .signature = "match(regex)", .doc = "Returns `{offset, length, string, captures: [...]}` for the first match. `offset` is a character count. Unmatched optional groups report `offset: -1`. `match(regex; \"g\")` is a generator that yields one match object per non-overlapping occurrence. No backrefs, no lookaround.", .category = .string },
    .{ .name = "sub", .arity = 2, .signature = "sub(regex; replacement)", .doc = "Replace the first match. Replacement supports `\\1..\\9` and `\\g<name>` backref substitution; pattern-level backrefs are unsupported.", .category = .string },
    .{ .name = "gsub", .arity = 2, .signature = "gsub(regex; replacement)", .doc = "Replace every match. Same replacement syntax and compat notes as `sub`.", .category = .string },
    .{ .name = "capture", .arity = 1, .signature = "capture(regex)", .doc = "Returns an object mapping each named capture group to its matched text (`{name: string}`). Unnamed groups are skipped. Unmatched optional named groups map to null. No backrefs, no lookaround.", .category = .string },
    .{ .name = "scan", .arity = 1, .signature = "scan(regex)", .doc = "Generator: yields one result per non-overlapping match. Without captures, yields the matched string; with captures, yields `[capture1, capture2, …]`. No backrefs, no lookaround.", .category = .string },
    .{ .name = "splits", .arity = 1, .signature = "splits(regex)", .doc = "Generator: yields each segment between non-overlapping regex matches (like `split(sep)` but the separator is a regex). Yields the prefix, each inter-match gap, and the trailing tail (including empty segments). Accepts an optional `splits(regex; flags)` form.", .category = .string },
